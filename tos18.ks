...
set pitchPID to PIDLOOP(0.6, 0.05, 0.00, -1, 1).
...
if phase = 0 {
	...
	when ship:velocity:surface:mag > 95 then {
		set mode to "lift off".
		set pitchMODE to "alt sea".
		set pitchTRIM to 0.4.
...
when phase = 1 then {
	...
	set geotarget to latlng( -12 - 3, -73.5 - 6).
...
when phase = 2 then {
	...
	set headingMODE to "geotarget".
	set geotarget to waypoint("Leetrey's Voyage"):geoposition.
...
lock throttle to 0.
brakes on.
// wait until ship:velocity:surface:mag < 10.0.
// setBrakes(100).
